    ...
    async function loadContext() {
      try {
        ...
        setErrorMessage(
          error instanceof Error
            ? error.message
            : "Nu s-a putut încărca pagina de introducere manuală."
        );
      } finally {
        if (!cancelled) {
          setIsLoading(false);
      ...
      <div className="participant-portal-shell">
        ...
        <section className="participant-portal-card">
          <div className="participant-portal-card__header">
            <h1>Furnizare date manual</h1>
            <p>
              Aceasta este pagina temporară pentru fluxul manual.
              Mai târziu aici vei pune formularul real de trimitere către
              <strong> POST /participant-access/submissions</strong>.
            </p>
          </div>
